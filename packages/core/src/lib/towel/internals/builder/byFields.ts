...
export async function byFields(
  ...
  return new Promise((resolveQueryByFields, rejectQueryByFields) => {
    ...
    params.push(table)
    const queryParams = queryBuilder(table, fields)
    warnings.concat(queryParams.warnings) // Carry warnings to parent

    // Since it is possible to feed the query builder bad fields, we need to make sure we are
    // being fed good values
    if (
      queryParams.params.length === 0 ||
      queryParams.params.indexOf(undefined) > -1
    ) {
      rejectQueryByFields(new Error('No valid fields supplied.'))
    }
