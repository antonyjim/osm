...
function FieldOptions() {
  const [shownTab, setShownTab]: [string, React.Dispatch<React.SetStateAction<string>>] = React.useState('existingFields')
    ...
    <div className="form-opt-left">
      ...
      <div className="form-opt-tabs">
        ...
        <div className="form-opt-tab-bodies">
          <div id="existingFields" style={{display: shownTab === 'existingFields' ? 'block': 'none'}}>
            <h1 >WTF</h1>
          </div>
          <div id="primitives"  style={{display: shownTab === 'primitives' ? 'block': 'none'}}>
          <h1 >WTFS</h1>
          </div>
        </div>
      </div>
